...
// See details at https://issues.slicer.org/view.php?id=4252
  ...
  {
    QPixmap pixmap;
    if (QFile::exists(":/SplashScreen.png"))
    {
      isGif = true;
    }
    else
    ...
    if (isGif)
    {
      splash = splashScreen.data();
      movie = new QMovie(":/SplashScreen.png", QByteArray(), splash);
      if (movie->isValid())
      {
        QObject::connect(movie, &QMovie::frameChanged, splash, [splash, movie, guiApp]()
        {
          QPixmap currentPixmap = movie->currentPixmap();
          if (guiApp)
          {
            currentPixmap.setDevicePixelRatio(guiApp->devicePixelRatio());
          }
          splash->setPixmap(currentPixmap);
          // Preserve the message
          splash->showMessage(splash->message(), Qt::AlignBottom | Qt::AlignHCenter);
        });
        movie->start();
      }
    }
