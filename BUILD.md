...
* Setting up your git account:
    ...
    * If not already done, email `FirstName LastName <firstname.lastname@LTrace.com>` to be granted access to
    the [LTrace/GeoSlicer](https://github.com/LTrace/GeoSlicer) repository.
...
mkdir W
cd /c/W
git clone https://github.com/LTrace/GeoSlicer.git GeoSlicer
```
...
1. Start [CMake GUI](https://cmake.org/runningcmake/), select source directory `C:\W\GeoSlicer` and set build directory to `C:\W\GeoSlicer-rel`.
2. Add an entry `Qt5_DIR` pointing to `C:/Qt/${QT_VERSION}/${COMPILER}/lib/cmake/Qt5`.
2. Generate the project.
3. Open `C:\W\GeoSlicer-rel\GeoSlicer.sln`, select `Release` and build the project.

<b>Option 2: Command Line</b>

1. Start the [Command Line Prompt](http://windows.microsoft.com/en-us/windows/command-prompt-faq)
2. Configure and build the project in `C:\W\GeoSlicer-rel` by typing the following commands:

```bat
cd C:\W\
mkdir GeoSlicer-rel
cd GeoSlicer-rel
cmake -G "Visual Studio 14 2015 Win64" -DQt5_DIR:PATH=`C:/Qt/${QT_VERSION}/${COMPILER}/lib/cmake/Qt5 ..\GeoSlicer
...
1. In the `C:\W\GeoSlicer-rel\Slicer-build` directory, open `Slicer.sln` and build the `PACKAGE` target
...
```bat
cd C:\W\GeoSlicer-rel\Slicer-build
